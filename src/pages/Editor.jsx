...
function Editor() {
  ...
  const handleAIApply = (aiItems, clearFirst) => {
    ...
    aiItems.forEach((aiItem, i) => {
      setTimeout(() => {
        const cat = FURNITURE_CATALOG.find(f => f.id === aiItem.catalogId)
        if (!cat) return
        addItem({ ...cat, _aiPosition: [aiItem.x, 0, aiItem.z], _aiRotation: aiItem.rotation || 0 })
      }, i * 250)
    })
  }
      ...
      <div className={`h-16 flex items-center justify-between px-6 border-b backdrop-blur-md z-10 shrink-0 ${hdr}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { setShowAI(v => !v); setShowCatalog(false); setMeasureActive(false) }}
            className={`ai-btn flex items-center gap-2 px-4 py-2 rounded-xl text-white text-sm font-semibold transition-all shadow-lg hover:scale-105 mr-1 ${showAI ? 'ring-2 ring-white/20' : ''}`}
          >
